"""
Graph Attention Transformer for Causal Inference in Network Settings.

This module implements a GAT-based model that:
1. Learns node representations from covariates using Graph Attention Networks
...
Architecture:
    ...
    GAT Encoder: Multiple GAT layers to learn representations Phi(X, A)
...
class GATEncoder(nn.Module):
    """
    Graph Attention Network encoder for learning node representations.
    
    Takes node features and graph structure, outputs learned representations.
    Uses multi-head attention to capture different aspects of node relationships.
    """
    
    def __init__(self, input_dim: int, hidden_dims: List[int] = [64, 32], 
                 output_dim: int = 16, heads: int = 4, dropout: float = 0.1, 
                 use_gatv2: bool = True):
        """
        Args:
            ...
            use_gatv2: if True, use GATv2Conv (improved dynamic attention)
        """
        super().__init__()
        
        if not TORCH_GEOMETRIC_AVAILABLE:
            raise ImportError(
                "torch_geometric is required for GAT. "
        ...
        for i, h_dim in enumerate(hidden_dims):
            ...
            # For intermediate layers: concat heads; for last layer: average
            out_per_head = h_dim // heads if not is_last else h_dim
            ...
            self.layers.append(
                GATLayer(
                    ...
                    heads=n_heads,
                    dropout=dropout,
                    concat=not is_last  # Concat for intermediate, average for last
                )
            )
...
# =============================================================================
# IPM Regularizer (Maximum Mean Discrepancy)
# =============================================================================
...
# =============================================================================
# Main Model: GAT Outcome Model
# =============================================================================

class GATOutcomeModel(nn.Module):
    """
    Graph Attention Transformer for outcome prediction in network causal inference.
    
    This model:
    1. Uses GAT to learn node representations from covariates and graph structure
    ...
    where MSE is computed only on observed (D, Y) pairs, and IPM encourages
    the learned representations to have similar distributions across exposure levels.
    """
    
    def __init__(self, input_dim: int, k: int,
                 gat_hidden_dims: List[int] = [64, 32],
                 rep_dim: int = 16,
        ...
        Args:
            ...
            dropout: dropout rate
            use_gatv2: use GATv2 (dynamic attention, recommended)
        ...
        # GAT encoder for learning representations
        ...
        # Print device info
        print(f"[GATOutcomeModel] Using device: {self.device}")
        if self.device.type == 'cuda':
            print(f"[GATOutcomeModel] GPU: {torch.cuda.get_device_name(0)}")
        print(f"[GATOutcomeModel] Exposure levels: 0 to {k-1} ({k} levels)")
    ...
    def predict_all_outcomes(self, phi: torch.Tensor) -> List[torch.Tensor]:
        """Predict outcomes Y(0), Y(1), ..., Y(k) for all exposure levels."""
    ...
    def fit(self, X: np.ndarray, A: np.ndarray, D: np.ndarray, Y: np.ndarray,
            ...
            val_split: float = 0.1,
            verbose: bool = True) -> 'GATOutcomeModel':
        """
        ...
        Args:
            ...
            D: exposure levels (n,), numpy array with values in {0, ..., k}
    ...
    def get_all_embeddings(self, X: np.ndarray, 
        ...
        Returns:
            X_GAT_root: shared backbone embeddings from GAT encoder (n x rep_dim)
            X_GAT_leaf: list of k embeddings, one per exposure level (n x last_hidden_dim)
        """
        if not self._fitted:
            raise ValueError("Model not fitted. Call fit() first.")
        ...
        with torch.no_grad():
            ...
            # Get root embeddings (shared backbone from GAT encoder)
            phi = self.encoder(X_t, edge_index)
            X_GAT_root = phi.cpu().numpy()
            
            # Get leaf embeddings (last hidden layer from each outcome head)
            X_GAT_leaf = []
            for d in range(self.k):
                leaf_emb = self.outcome_heads[d].get_last_hidden(phi)
                X_GAT_leaf.append(leaf_emb.cpu().numpy())
            
            return X_GAT_root, X_GAT_leaf
...
def fit_gat_outcome_model(X: np.ndarray, A: np.ndarray, D: np.ndarray, 
                          Y: np.ndarray, k: int, **kwargs) -> GATOutcomeModel:
    """
    Convenience function to fit a GAT outcome model.
    
    Args:
        X: node covariates (n x p)
        A: adjacency matrix (n x n)
        D: exposure levels (n,), values in {0, 1, ..., k}
        Y: observed outcomes (n,)
        k: maximum exposure level
        **kwargs: passed to GATOutcomeModel and fit()
    
    Returns:
        Fitted GATOutcomeModel
    """
    # Split kwargs between model init and fit
    model_kwargs = {
        'gat_hidden_dims': kwargs.pop('gat_hidden_dims', [64, 32]),
        'rep_dim': kwargs.pop('rep_dim', 16),
        'head_hidden_dims': kwargs.pop('head_hidden_dims', [32, 16]),
        'heads': kwargs.pop('heads', 4),
        'dropout': kwargs.pop('dropout', 0.1),
        'use_gatv2': kwargs.pop('use_gatv2', True),
    ...
    model = GATOutcomeModel(input_dim=X.shape[1], k=k, **model_kwargs)
...
if __name__ == "__main__":
    ...
    print("GAT-Embedder Example (out1 configuration)")
    print("=" * 60)
    ...
    print(f"Generating network with n={n}, avg_degree={avg_degree}, k={k}")
    ...
    # Use adjacency matrix G for GAT
    A = G.astype(float)
    ...
    # Get representations
    phi = model.get_node_representations(X)
